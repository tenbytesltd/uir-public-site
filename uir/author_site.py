...
for key, title, copy in (
    ("problem-design", "The design", "Fast to see, but semantics and implementation drift arrive later."),
    ("problem-code", "The code", "Executable, but the intent it lowered is usually absent."),
    ("problem-system", "The system", "Reusable values and Pieces, but not the whole interface or its unresolved meaning."),
):
...
node("mechanism-kicker", "paragraph", "ONE PACKAGE · TWO READABLE SIDES", parent=MECHANISM, salience="quiet", overrides={"type": "type-label", "ink": "ink-muted"})
node("mechanism-title", "heading", "See the drawing. Inspect the meaning. Approve both.", parent=MECHANISM, salience="primary")
...
for key, title, copy in (
    ("flow-input", "01 · Named evidence", "User words, product sources, and assets explicitly supplied for this run."),
    ("flow-package", "02 · One UIR package", "Visible facts, semantic facts, design system, provenance, and explicit gaps."),
    ("flow-prototype", "03 · Live projection", "A board and inspector that can be reproduced from the package alone."),
):
...
for key, title, copy, command in (
    ...
    ("quickstart-see", "2 · See", "Render the checklist, findings, and explicit remainder without turning silence into a pass.", "python3 tool/uir_report.py --change-set .uir/reading.json"),
    ("quickstart-adopt", "3 · Adopt", "Put the same measurement at the pull request, where design conformance is already being judged.", "# CI integration is alpha and remains an explicit adoption task"),
):
    card = node(key, "region", None, parent=STEPS, salience="secondary", overrides={"surface": "surface-code", "ink": "ink-inverse"})
    node(f"{key}-title", "heading", title, parent=card, salience="secondary", overrides={"type": "type-lead", "ink": "ink-inverse"})
    node(f"{key}-copy", "paragraph", copy, parent=card, overrides={"ink": "ink-inverse"})
    node(f"{key}-command", "code", command, parent=card)

BUILD = node("build", "region", None, parent=ROOT_NODE, salience="primary")
node("build-kicker", "paragraph", "THE SECOND PATH", parent=BUILD, salience="quiet", overrides={"type": "type-label", "ink": "ink-muted"})
node("build-title", "heading", "Start from words, not a blank canvas.", parent=BUILD, salience="primary")
...
for index, copy in enumerate((
    "Describe the job, audience, real content, and named evidence.",
    "Compile each confirmed decision into the one UIR package.",
    "Correct the visual board and semantic inspector conversationally.",
    "Freeze only after every required gap is explicit and both sides are accepted.",
), start=1):
...
node("standard-kicker", "paragraph", "BUILT TO EARN THE WORD STANDARD", parent=STANDARD, salience="quiet", overrides={"type": "type-label", "ink": "ink-muted"})
node("standard-title", "heading", "Open where trust depends on it. Strict where comparability depends on it.", parent=STANDARD, salience="primary")
...
for key, title, copy in (
    ("standard-source", "One source", "The interface and its compiled design system travel together."),
    ("standard-evidence", "Every claim has provenance", "Declared, derived, and inferred decisions never collapse into one confidence-free answer."),
    ("standard-gaps", "Silence is forbidden", "Missing, external, and waived work stays addressable and visible."),
    ("standard-targets", "Targets come later", "Frameworks lower a frozen UIR; they do not redefine it."),
):
...
for index, copy in enumerate((
    ...
    "Open now — contract gaps and deferred checks that still block the v0.1 freeze.",
    "Not claimed — a finished public install, production target compiler, or broad adopter ecosystem.",
), start=1):
...
node("footer-copy", "paragraph", "Created and stewarded by Tenbytes Ltd. The interface is the source. Code is a derived artifact.", parent=FOOTER, salience="quiet", overrides={"ink": "ink-inverse"}, sources=(SRC_USER, SRC_TENBYTES, SRC_REPO))
